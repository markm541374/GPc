...
names = ["../cache/tmp/PESIPS_"+str(dcc)+"_"+str(fls)+"_"+str(seed)+"_"+str(i)+".p" for i in xrange(nreps)]
...
for i,xs in enumerate(subset):
    ...
    names = ["../cache/tmp/PESIS_"+str(xs)+"_"+str(dcc)+"_"+str(fls)+"_"+str(seed)+"_"+str(k)+".p" for k in xrange(nreps)]
...
#a[2].set_xscale('log')   
plt.savefig('../figs/tmpexpIPS.png')
plt.show()
