...
Z = ESutils.draw_support_inplane(G, sp.array([-1.,-1.]),sp.array([1.,1.]),400,ESutils.SUPPORT_SLICELCB,1.,0.5)
...
Z = ESutils.draw_support_inplane(G, sp.array([-1.,-1.]),sp.array([1.,1.]),400,ESutils.SUPPORT_SLICELCB,0.,0.5)
R = ESutils.draw_min(G,Z,100)
for i in xrange(Z.shape[0]):
    plt.plot(Z[i,0],Z[i,1],'r.')
for i in xrange(R.shape[0]):
